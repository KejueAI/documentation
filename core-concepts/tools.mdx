...
Custom tools support three types of parameters:

- **Dynamic parameters** — values the AI agent fills in based on the conversation context. For example, the customer's order number or the issue description. These are visible to the AI.
- **Static parameters** — fixed values you configure once, like an API key or account ID. These are always sent with every request and are hidden from the AI.
- **System parameters** — values injected automatically from the execution context at runtime, like the current call ID or contact ID. These change per call/conversation and are hidden from the AI.

Parameters can be routed to different parts of the HTTP request: the request body, query string, URL path, or headers.

### System Parameters

System parameters let you pass execution context to your external APIs without exposing it to the AI. This is useful when your API needs to know _which_ call or contact triggered the request.
