...
If your agent enters a call from more than one starting state (cold dial, warm transfer, scheduled callback, return caller), give it a variable like <code>&#123;&#123;call_mode&#125;&#125;</code> and a distinct opener per mode. The worst tonal failure on a warm transfer is the receiving agent re-introducing the company and re-qualifying — the caller has already heard all that.
...
Pass the context as variables; render them via <code>&#123;&#123;var&#125;&#125;</code> in the prompt. See [Dynamic Prompt Variables](/guides/dynamic-variables) for call-start variables and [Inject Context](/guides/inject-context) for live updates mid-call.
...
The best prompts grow this way: a thin first draft, then twenty iterations of *"caller said X, agent did Y, that's wrong, add rule."*
